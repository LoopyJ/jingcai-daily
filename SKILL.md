...
name: jingcai-daily
description: >
  每日竞彩足球批量分析、赔率刷新和批量复盘工作流。用户明确要求按今天、明天或指定日期发现并分析竞彩集合、
  刷新整批当日赔率、临场复测，或复盘一个业务日/批次时使用。按中国竞彩业务日（Asia/Shanghai 当日11:00
  至次日11:00）获取并核验尚未开赛场次，逐场调用 soccer-predict，生成日期目录下的单场报告、
  结构化结果、Markdown 汇总报告、幂等历史归档，在对话中直接返回逐场核心预测，
  并在最终校验后同步预测记录到 GitHub。仅给出一个或多个明确 match ID、未要求上述日级操作时不要使用；
  交给 soccer-predict，比赛数量本身不触发本技能。日级多场任务优先使用独立写入范围的 subagent 并行分析。
---
...
- 仅给出一个或多个明确 `match_id`，且未点名本技能、未要求按业务日发现比赛、刷新整批赔率或生成日汇总时，
  统一交给 `$soccer-predict`；比赛数量本身不触发本技能。
