...
1. `predict_one.py start <match_id>`：建立独占目录，完成一次抓取与紧凑摘要；
2. 分析单元只读取 `summary.json`，按需补证并写小型 `analysis-overlay.json`；
3. `predict_one.py finish --run-dir <run_dir> --overlay <overlay> --mode batch`：
...
- 自己的独占临时目录（含 `run-state.json` 与临时 `chat.md`）
