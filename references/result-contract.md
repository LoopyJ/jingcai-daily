...
每个候选 match ID 都必须有一个 JSON。`success` 同时需要 Markdown；其他状态允许没有 Markdown。
...
```json
{
  ...
  "analysis_version": "soccer-predict v1.3.25",
  "recommendation": "赫根 -1",
  "probability": 0.56,
  "primary_direction": "赫根 -1",
  "action_status": "formal_standard",
  "stars": 2,
  "stake_cap": 0.75,
  "predicted_score": "2-0",
  "score_scenarios": {
...
- `soccer-predict v1.3.25` 或更高版本的新结果必须提供
  `primary_direction`、`action_status`、`stars` 和 `stake_cap`，供汇总报告与对话直返使用。
  `action_status` 只能是 `formal_standard|formal_cautious|direction_only`。
  `formal_cautious` 最高一星且 `stake_cap <= 0.25`；`direction_only` 必须为
  `stars=0`、`stake_cap=0`且 `formal_recommendation=false`。标准/谨慎正式则
  `formal_recommendation=true`。
...
- `shadow_forecast.ou.model_probability` 统一等于所选方向的
  ...
  `profit_probability=full_win+half_win`。两侧 EV 都不为正时，`direction` 和
